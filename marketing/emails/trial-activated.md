# Trial Activated Email

**Trigger**: User redeems a trial code or starts a free trial  
**From**: Stull Atlas <hello@stullatlas.app>  
**Subject**: Your Pro trial is live — here's where to start

---

Hi {{first_name}},

Your **{{trial_days}}-day Pro trial** is now active! Here's what you've unlocked:

**Blend Calculators**
- **Line Blend**: Interpolate between two glazes
- **Triaxial**: Explore 3-corner blends in a grid
- **Biaxial & Radial**: Advanced multi-material blends

**Optimizer**
- Set a target UMF and let the genetic algorithm find recipes that hit it

**AI Suggestions**
- Describe what you want — "a matte celadon at cone 6" — and get tailored recommendations

**Analysis Tools**
- DBSCAN clustering, density maps, void detection, surface fitting

[Start exploring Pro features →](https://stullatlas.app/#/explorer)

Your trial runs until **{{trial_end_date}}**. No credit card on file — it simply reverts to Free when it ends.

Questions? Reply to this email.

— The Stull Atlas Team

---

*[Unsubscribe]({{unsubscribe_url}})*
