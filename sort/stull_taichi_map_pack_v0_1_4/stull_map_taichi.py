#!/usr/bin/env python3
"""Taichi Stull Map Viewer (v0.1.4)

Adds:
- Simple histogram panel for current color metric (10 bins) with counts
- Guide labels (drawn at guide centroid / band corner if available)
- Export selected/pinned IDs to `selection.json` (E)

Controls (additions):
- E: export selection (selected + pins) to selection.json
"""
...
def centroid_polygon(poly):
    xs = [p["sio2"] for p in poly]
    ys = [p["al2o3"] for p in poly]
    if not xs or not ys:
        return None
...
def main():
    ...
    ap.add_argument("--pick-radius", type=float, default=0.02)
    ap.add_argument("--stem-scale", type=float, default=0.12)
    ap.add_argument("--screenshot", default="stull_map.png")
    ap.add_argument("--selection-out", default="selection.json")
    args = ap.parse_args()
    ...
    def export_selection():
        ids = []
        if selected >= 0:
            ids.append(pts[selected].get("id"))
        for i in pinned:
            pid = pts[i].get("id")
            if pid not in ids:
                ids.append(pid)
        payload = {"selected": (pts[selected].get("id") if selected >= 0 else None), "pinned": [pts[i].get("id") for i in pinned], "all": ids}
        with open(args.selection_out, "w", encoding="utf-8") as f:
            json.dump(payload, f, indent=2)

    # Histogram helper (CPU)
    ...
    while gui.running:
            ...
            for g in guides:
                ...
                if gtype == "polygon":
                    ...
                    if len(pts2) >= 2:
                        ...
                        ctd = centroid_polygon(pts2)
                        if ctd and label:
                            lx, ly = world_to_screen(ctd[0], ctd[1])
                            if 0.0 <= lx <= 1.0 and 0.0 <= ly <= 1.0:
                                gui.text(label[:18], pos=(lx, ly), color=0x2E86C1)
                elif gtype == "band":
                    band = g.get("band", {})
                    ...
                    if label:
                        lx, ly = world_to_screen(x0, y1)
                        if 0.0 <= lx <= 1.0 and 0.0 <= ly <= 1.0:
                            gui.text(label[:18], pos=(lx, ly), color=0x27AE60)
        ...
        if lod_active:
            ...
            # Build histogram values for current color metric (respect severity filter)
            hist_vals = []
            for i in range(n):
                s = int(sev[i])
                if sev_mode == 1 and s < 1:
                ...
                if sev_mode == 2 and s < 2:
                    continue
                colv, _ = get_color_height(i)
                hist_vals.append(colv)
            hist, (hlo, hhi) = histogram(hist_vals, bins=10)
            ...
            # Draw as text + tiny bars using circles
            gui.text("color hist (10 bins)", pos=(0.01, 0.08), color=0xFFFFFF)
            gui.text(f"range [{hlo:.2f}, {hhi:.2f}]  n={len(hist_vals)}", pos=(0.01, 0.05), color=0xFFFFFF)
            maxc = max(hist) if hist else 1
            maxc = max(1, maxc)
            for bi, cnt in enumerate(hist):
                bx = 0.01 + bi * 0.025
                by = 0.02
                bar = cnt / maxc
                # stack a few circles as a bar
                steps = int(1 + 8 * bar)
                for k in range(steps):
                    gui.circle(pos=(bx, by + k*0.008), radius=1.0/args.w, color=0xFFFFFF)
        ...
        gui.text(
            ...
            f"filter={sev_mode_names[int(sev_mode)]} stems={'on' if show_stems else 'off'} pins={len(pinned)}  (E export)",
            pos=(0.01, 0.98),
            color=0xFFFFFF
        )
        gui.show()
