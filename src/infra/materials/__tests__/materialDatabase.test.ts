...
describe('MaterialDatabase', () => {
  ...
  describe('resolve', () => {
    it('resolves exact primary name', () => {
      const mat = materialDatabase.resolve('EPK Kaolin')
      expect(mat).not.toBeNull()
      expect(mat!.primaryName).toBe('EPK Kaolin')
    })

    it('resolves by alias (case insensitive)', () => {
      const mat = materialDatabase.resolve('edgar plastic kaolin')
      expect(mat).not.toBeNull()
      expect(mat!.id).toBe('epk')
    })

    it('resolves by partial match (prefix)', () => {
      const mat = materialDatabase.resolve('Custer')
      expect(mat).not.toBeNull()
      expect(mat!.primaryName).toBe('Custer Feldspar')
    })

    it('resolves case-insensitively', () => {
      const mat = materialDatabase.resolve('SILICA')
      expect(mat).not.toBeNull()
      expect(mat!.category).toBe('silica')
    })

    it('returns null for empty string', () => {
      expect(materialDatabase.resolve('')).toBeNull()
    })
    ...
    it('returns null for unknown material', () => {
      expect(materialDatabase.resolve('XyzNotAMaterial')).toBeNull()
    })

    it('returns null for very short unknown strings', () => {
      expect(materialDatabase.resolve('ab')).toBeNull()
    })
  })
  ...
  describe('discontinued flag', () => {
    it('active materials have discontinued = false', () => {
      const silica = materialDatabase.resolve('Silica')
  ...
  // (getDatasets removed — single dataset architecture)
})
