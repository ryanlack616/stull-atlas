...
interface GlazeActions {
  ...
  // Get plot points (optionally recomputed for a molar weight set)
  getPlotPoints: (weightSetId?: MolarWeightSetId) => GlazePlotPoint[]
...
let _plotCache: { version: number; points: GlazePlotPoint[] } | null = null

/**
 * Precomputed molar-weight variant caches.
 * Key = weightSetId, value = { version, points }
...
export const useGlazeStore = create<GlazeState & GlazeActions>()(
  immer((set, get) => ({
    ...
    getPlotPoints: (weightSetId) => {
      const version = get()._version

      // ── Fast path: default weights (or no set specified) ──
      if (!weightSetId || weightSetId === 'app_default') {
        if (_plotCache && _plotCache.version === version) {
          return _plotCache.points
        }

        const points = buildPlotPoints(get().getGlazesArray())
        _plotCache = { version, points }

        // Also seed the molar cache for 'app_default'
        _molarCaches.set('app_default', { version, points })

        return points
      }

      // ── Molar-weight variant path ──
      const cached = _molarCaches.get(weightSetId)
      if (cached && cached.version === version) return cached.points

      // Ensure base points exist
      if (!_plotCache || _plotCache.version !== version) {
        const base = buildPlotPoints(get().getGlazesArray())
        _plotCache = { version, points: base }
        _molarCaches.set('app_default', { version, points: base })
      }
      ...
      const points = buildPlotPointsRecomputed(glazes, fromSet, weightSetId)
      _molarCaches.set(weightSetId, { version, points })
      return points
    },
...
function resolveUMF(g: GlazeRecipe) {
  return g.umf ?? null
}
 ...
 */
export function precomputeAllMolarVariants() {
  ...
  const base = buildPlotPoints(glazes)
  _plotCache = { version, points: base }
  _molarCaches.set('app_default', { version, points: base })
  _molarCaches.set('glazy_current', { version, points: base })

  // Recomputed variants
  const fromSet: MolarWeightSetId = 'glazy_current'
  for (const setId of getMolarWeightSetIds()) {
    if (setId === 'app_default' || setId === 'glazy_current') continue
    const points = buildPlotPointsRecomputed(glazes, fromSet, setId)
